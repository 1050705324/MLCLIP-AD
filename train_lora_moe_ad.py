...
CLASS_INDEX = {'Substation': 3, 'Substation': 2, 'Substation': 1, 'Substation': -1, 'Substation': -2, 'Substation': -3,
               'Substation': 4}
CLASS_INDEX_INV = {3: 'Substation', 2: 'Substation', 1: 'Substation', -1: 'Substation', -2: 'Substation',
                   -3: 'Substation', 4: 'Substation'}
...
def main():
    ...
    parser.add_argument('--obj', type=str, default='Substation')
    ...
    # LoRA arguments
    parser.add_argument('--r', default=256, type=int)
    parser.add_argument('--alpha', default=512, type=int)
    ...
    parser.add_argument('--save_path_lora', default='./ckpt/fine dataset/substation/just 1+1/256_512_4', help='path to save the lora modules after training, not saved if None')
    ...
    #scheduler_optimizer = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, total_iters, eta_min=1e-6)
    ...
    for epoch in range(args.epoch):
        ...
        for (image, image_label, mask, seg_idx) in tqdm(train_loader):
            if idx % (len(train_loader) // 5) == 0:
                ...
                if score >= save_score:
                    save_score = score
                    ckp_path = f'./ckpt/fine dataset/substation/just 1+1/256_512_4/{args.obj}.pth'
